<template>
  <v-form ref="form" v-model="valid" class="v-auth-form">
    ...
    <v-text-field v-model="form.email" class="mb-2" :error-messages="getError('email')" outlined label="Email" required @focus="clearError('email')" />
    <v-text-field v-if="!forgot" v-model="form.password" class="mb-2" type="password" :error-messages="getError('password')" outlined label="Password" required @focus="clearError('password')" />
    <div class="mb-6">
      <v-btn color="primary" :loading="loading" block @click="onSubmit"> {{ forgot ? 'Send password reset email' : 'Login' }} </v-btn>
    </div>
    <div class="mb-6">
    ...
    <div class="text-body-2 text-center">
      ...
      <a class="primary--text" @click.prevent="onSingup"> Sign up </a>
    </div>
  </v-form>
</template>
